...
class GCloudStorageConnector(BaseConnector):
    ...
    def _create_storage_client(self, action_result):
        ...
        try:
            ...
            self._client = googleapiclient.discovery.build(STORAGE_SERVICE_NAME, STORAGE_SERVICE_VERSION, credentials=credentials)
    ...
    def _send_request(self, request, action_result):
        try:
            ...
            return RetVal(action_result.set_status(phantom.APP_ERROR, f"Google API HTTP Error: {error_code}. {error_reason} "), None)
        except errors.Error as e:
            error_reason = self._get_error_message_from_exception(e)

            return RetVal(action_result.set_status(phantom.APP_ERROR, f"Google API Request Error. {error_reason}"), None)
    ...
    def _handle_list_objects(self, param):
        ...
        summary = {"num_objects": len(response.get("items", []))}
        ...
        if download_file:
            ...
            with open(file_path, "wb") as f:
                ...
                while not done:
                    ...
                    if progress:
                        self.save_progress(f"File Download: {progress.progress() * 100}")
            try:
                ...
                err = self._get_error_message_from_exception(e)
                ret_val = action_result.set_status(phantom.APP_ERROR, f"Could not add file to vault. {err}")
    ...
    def _handle_create_object(self, param):
        ...
        try:
            ...
            self.debug_print(f"Rules.vault_info results: success: {success}, message: {message}, info: {vault_info}")
        except requests.exceptions.HTTPError:
            error_message = f"Invalid Vault ID: {vault_id}"
            return action_result.set_status(phantom.APP_ERROR, error_message)
        except Exception as e:
            err = self._get_error_message_from_exception(e)
            return action_result.set_status(phantom.APP_ERROR, f"Error opening file. {err}")
        ...
        try:
            ...
            err = self._get_error_message_from_exception(e)
            return action_result.set_status(phantom.APP_ERROR, f"Error occurred while getting 'File Info'. {err}")
    ...
    def _handle_list_buckets(self, param):
        ...
        summary = {"total_objects": len(response.get("items", []))}
    ...
    def _handle_create_bucket(self, param):
        ...
        req_body = {"name": bucket, "location": location}
